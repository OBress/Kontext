...
export default function SettingsPage() {
    ...
    fetch("/api/settings/preferences")
      ...
      .then((data) => {
        if (data.preferences?.activity_filters) {
          setActivityFilters({
            ...DEFAULT_ACTIVITY_FILTERS,
            ...data.preferences.activity_filters,
          });
        }
      })
      .catch(() => {})
  ...
  const KONTEXT_EVENTS = KONTEXT_ACTIVITY_EVENTS;

  const GITHUB_EVENTS = GITHUB_ACTIVITY_EVENTS;
        ...
        <div className="grid grid-cols-2 gap-5 items-start">
          ...
          <div className="flex flex-col gap-5">
              ...
              {!editingKey ? (
                <div className="space-y-2">
                  <div className="flex items-center gap-2 px-3 py-1.5 rounded-lg bg-[var(--surface-1)] border border-[var(--alpha-white-5)]">
                    <span className="font-mono text-xs text-[var(--gray-300)] flex-1 truncate">
                  ...
                  <button onClick={() => { setEditingKey(true); setKeyInput(apiKey || ""); }} className="px-3 py-1.5 rounded-lg text-xs font-mono bg-[var(--alpha-white-5)] text-[var(--gray-300)] hover:text-[var(--gray-100)] border border-[var(--alpha-white-8)] cursor-pointer">
                    {apiKey ? "Change Key" : "Add Key"}
                  </button>
                </div>
              ) : (
                <div className="space-y-2">
                  <input type="password" value={keyInput} onChange={(e) => { setKeyInput(e.target.value); setTestStatus("idle"); setTestResult(null); setTestError(null); }} placeholder="AIza..." className="w-full px-3 py-1.5 rounded-lg font-mono text-xs bg-[var(--surface-1)] border border-[var(--alpha-white-5)] text-[var(--gray-200)] placeholder:text-[var(--gray-600)] focus:outline-none focus:border-[var(--accent-green)]/40" />
                  <div className="flex gap-2">
                    <button onClick={handleTest} disabled={!keyInput || testStatus === "testing"} className="flex items-center gap-1.5 px-3 py-1.5 rounded-lg text-xs font-mono bg-[var(--alpha-white-5)] text-[var(--gray-300)] border border-[var(--alpha-white-8)] cursor-pointer disabled:opacity-40">
                      {testStatus === "testing" ? <Loader2 size={12} className="animate-spin" /> : testStatus === "success" ? <Check size={12} className="text-[var(--accent-green)]" /> : null}
                      Test
                    </button>
                    <button onClick={handleSaveKey} className="px-3 py-1.5 rounded-lg text-xs font-mono bg-[var(--accent-green)] text-black font-medium cursor-pointer border-none hover:opacity-90">Save</button>
                    <button onClick={() => setEditingKey(false)} className="px-3 py-1.5 rounded-lg text-xs font-mono bg-transparent text-[var(--gray-500)] cursor-pointer border border-[var(--alpha-white-8)]">Cancel</button>
                  </div>
            ...
            <GlowCard glowColor="green" className="p-5">
              ...
              <p className="font-mono text-xs text-[var(--gray-500)] mb-3 m-0">
                Choose which repo, sync, team, and GitHub events appear in your dashboard activity feed.
              </p>

              {filtersLoading ? (
                <div className="space-y-2">
                  {[...Array(4)].map((_, i) => (
                    <div key={i} className="h-7 rounded bg-[var(--alpha-white-5)] animate-pulse" />
                  ))}
                </div>
              ) : (
                  ...
                  <div>
                    ...
                    <div className="space-y-0">
                      {KONTEXT_EVENTS.map(({ key, label }) => (
                        <label
                          key={key}
                          className="flex items-center justify-between px-3 py-1.5 rounded-lg hover:bg-[var(--alpha-white-5)] transition-colors cursor-pointer"
                        >
          ...
          <div className="flex flex-col gap-5">
            ...
            <GlowCard glowColor="green" className="p-5">
              ...
              {newRawKey && (
                <div className="mb-3 p-3 rounded-lg bg-[var(--accent-green)]/10 border border-[var(--accent-green)]/20">
              ...
              <div className="flex gap-2 mb-3">
                <input
                  ...
                  className="flex-1 px-3 py-1.5 rounded-lg text-xs font-mono bg-[var(--surface-1)] border border-[var(--alpha-white-5)] text-[var(--gray-200)] placeholder:text-[var(--gray-600)] focus:outline-none focus:border-[var(--accent-green)]/40"
                />
                <button
                  onClick={handleCreateMcpKey}
                  disabled={!newKeyName.trim() || creatingKey}
                  className="flex items-center gap-1.5 px-3 py-1.5 rounded-lg text-xs font-mono bg-[var(--accent-green)] text-black border-none cursor-pointer hover:opacity-90 disabled:opacity-40"
                >
              ...
              <div className="space-y-2">
                {mcpKeys.length === 0 ? (
                  <p className="font-mono text-xs text-[var(--gray-600)] text-center py-3">No API keys created yet</p>
                ) : (
                  mcpKeys.map((key) => (
            ...
            <GlowCard glowColor="none" className="p-5">
              <h3 className="font-mono text-sm font-medium text-[var(--gray-200)] mb-3 m-0">Account</h3>
              <form action={signOut}>
